...
<head>
    ...
    <title>Sallahly.dz | Dashboard | Ventes </title>
</head>
            ...
            <div class="w-full  ml-[90px] p-10  flex-1">
                <div class="rounded-[4px] relative w-full ">
                    <div class="container w-full h-auto bg-white border-[1px] rounded-[4px] flex items-center justify-between border-gray-300 p-3 mx-auto">
                        <h1 class="text-[20px] font-bold">Commandes vente</h1>
                    </div>
                        ...
                        <table class="w-full max-w-full mb-4 max-h-[500px] hide-scrollbar  bg-transparent table-bordered">
                            <thead class="w-full font-semibold h-12 bg-[#7b7b8a] sticky z-[100]  top-0 left-0 text-white rounded-[4px] ">
                                <tr class="w-full h-full rounded-[4px]">
                                    ...
                                    <th class="relative flex-grow w-1/5 flex-1 px-4">E-mail client</th>
                                    <th class="relative flex-grow w-1/5 flex-1 px-4">Telephone</th>
                                    <th class="relative flex-grow w-1/5 flex-1 px-4">Commande</th>
                                    <th class="relative flex-grow w-1/5 flex-1 px-4">Addresse</th>
                                    <th class="relative flex-grow w-1/5 flex-1 px-4">Mode peiment </th>
                            ...
                            <tbody class="w-full py-2">
                                ...
                                $allOredrs = $conn->query($GetAllOrders);
                                if ($allOredrs->num_rows > 0) {
                                    $order = mysqli_fetch_all($allOredrs, MYSQLI_ASSOC);
                                    foreach ($order as $order) {
                                ?>
                                        <tr class="w-full h-12 py-2">
                                            <td class="relative flex-grow w-1/5 items-center justify-center text-center flex-1 px-4"><?php echo $order['id']; ?></td>
                                            <td class="relative flex-grow w-1/5 items-center justify-center text-center flex-1 px-4"><?php echo $order['firstname']; ?> <?php echo $order['lastname']; ?></td>
                                            <td class="relative flex-grow w-1/5 items-center justify-center text-center flex-1 px-4"><?php echo $order['email']; ?></td>
                                            <td class="relative flex-grow w-1/5 items-center justify-center text-center flex-1 px-4"><?php echo $order['phone']; ?></td>
                                            <td class="relative flex-grow w-1/5 items-center justify-center text-center flex-1 px-4"><?php echo $order['products']; ?></td>
                                            <td class="relative flex-grow w-1/5 items-center justify-center text-center flex-1 px-4"><?php echo $order['amount_paid']; ?></td>
                                            <td class="relative flex-grow w-1/5 items-center justify-center text-center flex-1 px-4"><?php echo $order['uaddress']; ?></td>
                                            <td class="relative flex-grow w-1/5 items-center justify-center text-center flex-1 px-4"><?php echo $order['pmode']; ?></td>
                                            <td class="relative flex-grow  items-center justify-end text-center">
                                                <div class="w-full h-full px-2 py-1 flex items-center justify-between">
                                                    <form method="POST" action="">
                                                        <button class="inline-block align-middle text-center select-none cursor-pointer border font-semibold text-[14px] whitespace-no-wrap rounded-md py-3 px-10 leading-normal no-underline bg-[#FB7185] text-white hover:bg-[#E11D48]" type="submit" name="delete_order" value="<?= $order['id']; ?>">
                                                            Supprimer
                                                        </button>
                                                    </form>
                                                </div>
