...
<head>
    ...
    <title>Sallahly.dz | Techniciens </title>
</head>
...
if (isset($_POST['delete_user'])) {
    ...
    $selectedUser = mysqli_query($conn, "DELETE FROM `users` WHERE id = '$user_id' AND role not in ('user', 'admin')") or die('La requête a échoué');
    if ($selectedUser) {
        $_SESSION['message_success'] = 'Technicien Supprimer avec succès!';
        header("Location: techniciens.php");
        exit(0);
    } else {
        $_SESSION['message_error'] = 'La Suppression du technicien à échoué!';
        exit(0);
    }
}
?>
            ...
            <div class="w-full  ml-[90px] p-10  flex-1">
                <div class="rounded-[4px] relative w-full ">
                    <div class="container w-full h-auto bg-white border-[1px] rounded-[4px] flex items-center justify-between border-gray-300 p-3 mx-auto">
                        <h1 class="text-[20px] font-bold">Liste des techniciens</h1>
                    </div>
                        ...
                        <table class="w-full max-w-full mb-4 max-h-[500px] hide-scrollbar  bg-transparent table-bordered">
                            <thead class="w-full font-semibold h-12 bg-[#7b7b8a] sticky z-[100]  top-0 left-0 text-white rounded-[4px] ">
                                <tr class="w-full h-full rounded-[4px]">
                                    ...
                                    <th class="relative flex-grow w-1/5 flex-1 px-4">Diplomé</th>
                                    <th class="relative flex-grow w-1/5 flex-1 px-4">Catégorie</th>
                                    <th class="relative flex-grow w-1/5 flex-1 px-4">Justificatif</th>
                                    <th class="relative flex-grow w-1/5 flex-1 px-4">Actions</th>
                                </tr>
                            </thead>
                            <tbody class="w-full py-2">
                                <?php
                                $GetAllUsers = "SELECT * FROM users WHERE role not in ('user','admin')";
                                ...
                                ?>
                                        <tr class="w-full h-12 py-2">
                                            ...
                                            <td class="relative flex-grow w-1/5 items-center justify-center text-center flex-1 px-4"><?= $user['role'] == 'graduated' ? 'Oui' : 'Non'; ?></td>
                                            <td class="relative flex-grow w-1/5 items-center justify-center text-center flex-1 px-4"><?= $user['category']; ?></td>
                                            <td class="relative flex-grow w-1/5 items-center justify-center flex-1 px-4 text-center">
                                                <a href='<?= "/Sallahly/public/uploads/" . ($user['role'] == 'graduated' ? 'diplome/' : 'cv/') . $user['image'] ?>' class="underline text-blue-600" target="_blank" rel="noopener">
                                                    <?= $user['role'] == 'graduated' ? 'Diplome' : 'CV'; ?>
                                                </a>
                                            </td>
                                            <td class="relative flex-grow  items-center justify-end text-center">
                                                <div class="w-full h-full px-2 py-1 flex items-center justify-between">
                                                    <a href="affect-category.php?technicien_id=<?= $user['id']; ?>" class="inline-block align-middle text-center select-none cursor-pointer border font-semibold text-[14px] whitespace-no-wrap rounded-md py-3 px-10 leading-normal no-underline bg-[#06B6D4] text-white hover:bg-[#0E7490]">
                                                        Categorie
                                                    </a>
